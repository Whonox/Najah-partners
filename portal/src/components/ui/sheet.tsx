...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      ...
      className={cn(
        // `bg-overlay` et non `bg-black` : la couleur du voile vient du thème (src/index.css),
        // seule façon de le garder visible en clair COMME en sombre.
        "fixed inset-0 z-50 bg-overlay/25 transition-opacity duration-150 data-ending-style:opacity-0 data-starting-style:opacity-0 supports-backdrop-filter:backdrop-blur-xs",
        className
      )}
      {...props}
    />
