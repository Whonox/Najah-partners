...
const NETWORK: Array<{
  ...
  { code: 'NP000965', firstName: 'Niveau2D', upline: ROOT_CODE, leg: Leg.RIGHT },
  { code: 'NP000966', firstName: 'Niveau3GG', upline: 'NP000964', leg: Leg.LEFT },
  { code: 'NP000967', firstName: 'Niveau3GD', upline: 'NP000964', leg: Leg.RIGHT },
  { code: 'NP000968', firstName: 'Niveau3DG', upline: 'NP000965', leg: Leg.LEFT },
  { code: 'NP000969', firstName: 'Niveau3DD', upline: 'NP000965', leg: Leg.RIGHT },
];
...
export class SeedService {
  ...
  private async bootstrapNetwork(): Promise<void> {
    ...
    if (root) {
      this.logger.log(`Réseau d’amorçage déjà en place (${ROOT_CODE}) — rien à faire.`);
      return;
    }
    ...
    await this.prisma.$executeRaw`SELECT setval('member_code_seq', ${SEED_LAST_MEMBER_NUMBER - NETWORK.length}::bigint, true)`;
    ...
    for (const node of NETWORK) {
      const id = node.upline
        ? await this.registerSeedMember(node.code, node.firstName, node.upline, node.leg!)
  ...
  private async createRootMember(code: string, firstName: string): Promise<number> {
    const passwordHash = await bcrypt.hash(SEED_PASSWORD, this.bcryptRounds());
