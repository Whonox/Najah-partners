...
- Ordre inter-tables à respecter quand plusieurs sont touchées : `Member` (id croissant) → `Ecard` (**id croissant**) → `Product` (id croissant) → `Order` (D-027, Tranche 6 ; étendu en Tranche 7.5). Le checkout prend ses verrous dans cet ordre : l'activation d'abord (chaîne d'ancêtres, puis e-cards brûlées), le stock ensuite, la commande insérée en dernier. **Depuis le cumul de plusieurs e-cards par paiement (D-030/D-041), l'ordre par id croissant vaut AUSSI entre cartes** : sans lui, deux paiements concurrents tenant chacun une carte que l'autre convoite s'interbloqueraient. Même règle, même raison que pour les ancêtres et les produits.
