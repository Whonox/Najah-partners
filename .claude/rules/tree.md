...
- Ordre inter-tables à respecter quand plusieurs sont touchées : `Member` (id croissant) → `Ecard` → `Product` (id croissant) → `Order` (D-027, Tranche 6). Le checkout prend ses verrous dans cet ordre : l'activation d'abord (chaîne d'ancêtres, puis e-card brûlée), le stock ensuite, la commande insérée en dernier.
